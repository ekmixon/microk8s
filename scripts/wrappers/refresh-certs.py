...
def check_certificate():
    ...
    """
    cmd = f"{snap_path}/usr/bin/openssl x509 -enddate -noout -in {snapdata_path}/certs/ca.crt"
...
def undo_refresh():
    ...
    try:
        ...
        subprocess.check_call(
            f"cp -r {backup_dir}/credentials {snapdata_path}".split()
        )
...
def update_configs():
    ...
    p = subprocess.Popen(
        [
            "bash",
            "-c",
            f". {snap_path}/actions/common/utils.sh; update_configs",
        ]
    )
...
def take_backup():
    ...
    try:
        ...
        subprocess.check_call(
            f"cp -r {snapdata_path}/credentials {backup_dir}".split()
        )
...
def produce_certs():
    ...
    subprocess.check_call(
        f"rm -rf {snapdata_path}/certs/front-proxy-ca.crt".split()
    )
...
def install_certs(ca_dir):
    ...
    p = subprocess.Popen(
        [
            "bash",
            "-c",
            f". {snap_path}/actions/common/utils.sh; gen_server_cert",
        ]
    )
...
def validate_certificates(ca_dir):
    ...
    try:
        cmd = f"{snap_path}/usr/bin/openssl rsa -in {ca_dir}/ca.key -check -noout -out /dev/null"
    ...
    try:
        cmd = f"{snap_path}/usr/bin/openssl x509 -in {ca_dir}/ca.crt -text -noout -out /dev/null"
